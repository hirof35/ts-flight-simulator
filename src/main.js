...
class Vector2D {
    ...
    scale(n) { return new Vector2D(this.x * n, this.y * n); }
...
class Airplane {
    constructor() {
        // 状態量
        this.position = new Vector2D(100, 200); // 位置 (x, y) ※yは高度
        this.velocity = new Vector2D(30, 0); // 速度ベクトル (vx, vy) ※初期前進速度 30m/s
        this.pitch = 0; // 現在の機首方位 (ラジアン)
        this.targetPitch = 0; // マウス入力による目標機首方位
        this.throttle = 0.7; // スロットル (0.0 〜 1.0)
        this.isStalled = false; // 失速フラグ
        // 機体諸元（セスナ172をベースとした定数）
        this.mass = 1200; // 重量 (kg)
        this.wingArea = 16.2; // 翼面積 (m2)
        this.maxThrust = 3500; // 最大推力 (N)
        this.rho = 1.225; // 空気密度 (kg/m3)
        this.g = 9.81; // 重力加速度 (m/s2)
    }
    update(dt) {
        // --- 操縦応答（入力に対する機首回転の慣性） ---
...
class SimulatorRenderer {
    ...
    drawHUD(airplane, mouseX, mouseY) {
        // 文字情報
